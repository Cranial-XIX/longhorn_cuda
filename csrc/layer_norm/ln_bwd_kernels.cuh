...
__device__ __inline__ compute_t sigmoid(compute_t x) {
    return 1.f / (1.f + exp(-x));
}

template<typename compute_t>
__device__ __inline__ compute_t silu(compute_t x) {
    return x / (1.f + exp(-x));
}

template<typename compute_t>
__device__ __inline__ compute_t dsilu(compute_t dy, compute_t x) {
    compute_t sig_z = 1.f / (1.f + exp(-x));
    return dy * sig_z * (1 + x - x * sig_z);
}
    ...
    for( int row = r; row < params.rows; row += params.ctas_per_col * ROWS_PER_CTA ) {
        ...
        for( int it = 0; it < LDGS; it++ ) {
            ...
            for( int jt = 0; jt < NUM_ELTS; jt++ ) {
                ...
                compute_t g_tmp = compute_t(g[it].data.elt[jt]);

                compute_t y_tmp = rs_r * x_or_z_tmp;
                compute_t _dz_tmp = compute_t(dz[it].data.elt[jt]);
                compute_t dz_tmp = _dz_tmp * silu(g_tmp);
                compute_t dy_tmp = dz_tmp * gamma_tmp;

                dg[it].data.elt[jt] = dsilu(_dz_tmp * y_tmp * gamma_tmp, g_tmp);
                // dy_tmp *= g_tmp * sig_z;
