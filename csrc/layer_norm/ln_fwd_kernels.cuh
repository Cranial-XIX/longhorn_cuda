...
void ln_fwd_kernel(FwdParams params) {
    ...
    for( int row = r; row < params.rows; row += params.ctas_per_col * ROWS_PER_CTA ) {
        ...
        compute_t rs = rsqrtf(rn * m2 + params.epsilon + mu * mu);
        ...
        for( int it = 0; it < LDGS; it++ ) {
            ...
            #pragma unroll
            for( int jt = 0; jt < NUM_ELTS; jt++ ) {
                compute_t g_ij = compute_t(g[it].data.elt[jt]);
                compute_t y_ij = compute_t(rs * (xf[it * NUM_ELTS + jt]));
                z[it].data.elt[jt] = output_t(compute_t(gamma[it].data.elt[jt]) * y_ij * silu(g_ij));
            }
