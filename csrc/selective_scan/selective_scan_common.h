...
////////////////////////////////////////////////////////////////////////////////////////////////////
inline __device__ float softplus(const float a, const float threshold=20.f) {
    return a > threshold ? a : log1pf(expf(a));
}

inline __device__ float dsoftplus(const float a, const float dout, const float threshold=20.f) {
    return a <= 20.f
                ? dout / (1.f + expf(-a))
                : dout;
}

inline __device__ float sigmoid(const float x) {
    return 1.f / (1.0f + expf(-x));
}

inline __device__ float sigmoid(const float x, const float multiplier) {
    return multiplier / (1.0f + expf(-x));
}

inline __device__ float dsigmoid(const float y) {
    return y * (1.f - y);
}

inline __device__ float dtanh(const float y) {
    return (1.f - y * y);
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
template<typename Ktraits, bool kIsComplex>
