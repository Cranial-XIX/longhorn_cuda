...
template<int kNThreads_, int kNItems_, bool kIsEvenLen_, bool kIsVariableA_, bool kIsVariableB_, bool kIsVariableC_,
         bool kDeltaSoftplus_, bool kHasZ_, typename input_t_, typename weight_t_>
struct Selective_Scan_new_bwd_kernel_traits {
    static_assert(kNItems_ % 4 == 0);
    using input_t = input_t_;
    using weight_t = weight_t_;
    static constexpr int kNThreads = kNThreads_;
    ...
    static constexpr bool kIsComplex = std::is_same_v<weight_t, complex_t>;
    static constexpr bool kIsEvenLen = kIsEvenLen_;
    static constexpr bool kIsVariableA = kIsVariableA_;
    static constexpr bool kIsVariableB = kIsVariableB_;
    static constexpr bool kIsVariableC = kIsVariableC_;
    static constexpr bool kDeltaSoftplus = kDeltaSoftplus_;
    static constexpr bool kHasZ = kHasZ_;
    ...
    static constexpr int kSmemIOSize = std::max({sizeof(typename BlockLoadT::TempStorage),
                                                 sizeof(typename BlockLoadVecT::TempStorage),
                                                 (int(kIsVariableA) + int(kIsVariableB) + int(kIsVariableC)) * sizeof(typename BlockLoadWeightT::TempStorage),
                                                 (int(kIsVariableA) + int(kIsVariableB) + int(kIsVariableC)) * sizeof(typename BlockLoadWeightVecT::TempStorage),
                                                 sizeof(typename BlockStoreT::TempStorage),
                                                 sizeof(typename BlockStoreVecT::TempStorage)});
    static constexpr int kSmemExchangeSize = (int(kIsVariableA) + int(kIsVariableB) + int(kIsVariableC)) * sizeof(typename BlockExchangeT::TempStorage);
    static constexpr int kSmemReduceSize = sizeof(typename BlockReduceT::TempStorage);
    static constexpr int kSmemSize = kSmemIOSize + kSmemExchangeSize + kSmemReduceSize + sizeof(typename BlockScanT::TempStorage) + sizeof(typename BlockReverseScanT::TempStorage);
};
...
struct Selective_Scan_decay_bwd_kernel_traits {
    static_assert(kNItems_ % 4 == 0);
    using input_t = input_t_;
    using weight_t = weight_t_;
    static constexpr int kNThreads = kNThreads_;
    ...
    static constexpr int kSmemIOSize = std::max({sizeof(typename BlockLoadT::TempStorage),
                                                 sizeof(typename BlockLoadVecT::TempStorage),
                                                 4 * sizeof(typename BlockLoadWeightT::TempStorage),
                                                 4 * sizeof(typename BlockLoadWeightVecT::TempStorage),
                                                 sizeof(typename BlockStoreT::TempStorage),
                                                 sizeof(typename BlockStoreVecT::TempStorage)});
    static constexpr int kSmemExchangeSize = 4 * sizeof(typename BlockExchangeT::TempStorage);
    static constexpr int kSmemReduceSize = sizeof(typename BlockReduceT::TempStorage);
    static constexpr int kSmemSize = kSmemIOSize + kSmemExchangeSize + kSmemReduceSize + sizeof(typename BlockScanT::TempStorage) + sizeof(typename BlockReverseScanT::TempStorage);
};
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            if constexpr (!kIsVariableB) {
                ...
                load_weight<Ktraits, kIsComplex>(Bvar + state_idx * params.B_dstate_stride, B_vals,
            ...
            if constexpr (!kIsVariableC) {
                ...
                load_weight<Ktraits, kIsComplex>(Cvar + state_idx * params.C_dstate_stride, C_vals,
...
template<typename Ktraits>
__global__ __launch_bounds__(Ktraits::kNThreads, Ktraits::kMinBlocks)
void selective_scan_new_bwd_kernel(SSMNewParamsBwd params) {
    constexpr bool kIsComplex = Ktraits::kIsComplex;
    constexpr bool kIsVariableA = Ktraits::kIsVariableA;
    constexpr bool kIsVariableB = Ktraits::kIsVariableB;
    constexpr bool kIsVariableC = Ktraits::kIsVariableC;
    constexpr bool kDeltaSoftplus = Ktraits::kDeltaSoftplus;
    constexpr bool kHasZ = Ktraits::kHasZ;
    ...
    weight_t *dA = reinterpret_cast<weight_t *>(params.dA_ptr)
        + (!kIsVariableA ? dim_id * params.dA_d_stride : batch_id * (!kIsComplex ? params.dA_batch_stride : params.dA_batch_stride / 2) + group_id * params.dA_group_stride);
    weight_t *dB = reinterpret_cast<weight_t *>(params.dB_ptr)
        + (!kIsVariableB ? dim_id * params.dB_d_stride : batch_id * (!kIsComplex ? params.dB_batch_stride : params.dB_batch_stride / 2) + group_id * params.dB_group_stride);
    weight_t *dC = reinterpret_cast<weight_t *>(params.dC_ptr)
        + (!kIsVariableC ? dim_id * params.dC_d_stride : batch_id * (!kIsComplex ? params.dC_batch_stride : params.dC_batch_stride / 2) + group_id * params.dC_group_stride);
    ...
    delta += (params.n_chunks - 1) * kChunkSize;
    dout += (params.n_chunks - 1) * kChunkSize;
    Avar += (params.n_chunks - 1) * kChunkSize * (!kIsComplex ? 1 : 2);
    Bvar += (params.n_chunks - 1) * kChunkSize * (!kIsComplex ? 1 : 2);
    Cvar += (params.n_chunks - 1) * kChunkSize * (!kIsComplex ? 1 : 2);
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        input_t u_vals[kNItems];
        ...
        __syncthreads();
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            weight_t A_scaled, A_val, B_val, C_val;
            ...
            if constexpr (!kIsVariableA) {
                ...
                auto &smem_load_weight_A = !kIsVariableB ? (!kIsVariableC ? smem_load_weight : smem_load_weight1) : (!kIsVariableC ? smem_load_weight1 : smem_load_weight2);
                load_weight<Ktraits, kIsComplex>(Avar + state_idx * params.A_dstate_stride, A_vals,
                    smem_load_weight_A, (params.seqlen - chunk * kChunkSize) * (!kIsComplex ? 1 : 2));
            }
            // const weight_t A_val = smem_a[state_idx];
            scan_t thread_data[kNItems], thread_reverse_data[kNItems];
            if constexpr (!kIsComplex) {
                ...
                if (threadIdx.x == 0) { smem_running_postfix[state_idx] = postfix_op.running_prefix; }
                weight_t dA_val = 0, dBC_val = 0;
                weight_t dA_vals[kNItems], dB_vals[kNItems], dC_vals[kNItems];
                #pragma unroll
                for (int i = 0; i < kNItems; ++i) {
                    if constexpr (kIsVariableA) {
                        A_val = A_vals[i];
                    ...
                    const float a = thread_data[i].y - (!kIsVariableB ? delta_vals[i] * float(u_vals[i]) : delta_vals[i] * float(u_vals[i]) * B_vals[i]);
                    float dxa = dx * a;
                    ddelta_vals[i] += ddelta_u * float(u_vals[i]) + A_val * dxa; 
                    if constexpr (!kIsVariableB || !kIsVariableC) {
                        if constexpr (!kIsVariableB) {  // dBC_val is dB_val
                            dBC_val += dout_vals[i] * (!kIsVariableC ? thread_data[i].y : thread_data[i].y * C_vals[i]);
                        } else {  // dBC_val is dC_val
                    ...
                    if constexpr (kIsVariableA) {
                        dA_vals[i] = delta_vals[i] * dxa;
                    } else {
                        dA_val += delta_vals[i] * dxa;
                    }
                }
                // Block-exchange to make the atomicAdd's coalesced, otherwise they're much slower
                if constexpr (kIsVariableB || kIsVariableC || kIsVariableA) {
                    if constexpr (kIsVariableB) {
                        Ktraits::BlockExchangeT(smem_exchange).BlockedToStriped(dB_vals, dB_vals);
                    }
                    if constexpr (kIsVariableC) {
                    ...
                    if constexpr (kIsVariableA) {
                        auto &smem_exchange_A = !kIsVariableC ? (!kIsVariableB ? smem_exchange : smem_exchange1) : (!kIsVariableB ? smem_exchange1 : smem_exchange2);
                        Ktraits::BlockExchangeT(smem_exchange_A).BlockedToStriped(dA_vals, dA_vals);
                    }
                    const int seqlen_remaining = params.seqlen - chunk * kChunkSize - threadIdx.x;
                    weight_t *dA_cur = dA + state_idx * params.dA_dstate_stride + chunk * kChunkSize + threadIdx.x;
                    weight_t *dB_cur = dB + state_idx * params.dB_dstate_stride + chunk * kChunkSize + threadIdx.x;
                    weight_t *dC_cur = dC + state_idx * params.dC_dstate_stride + chunk * kChunkSize + threadIdx.x;
                    #pragma unroll
                    for (int i = 0; i < kNItems; ++i) {
                        if (i * kNThreads < seqlen_remaining) {
                            if constexpr (kIsVariableA) { gpuAtomicAdd(dA_cur + i * kNThreads, dA_vals[i]); }
                            if constexpr (kIsVariableB) { gpuAtomicAdd(dB_cur + i * kNThreads, dB_vals[i]); }
                            if constexpr (kIsVariableC) { gpuAtomicAdd(dC_cur + i * kNThreads, dC_vals[i]); }
                        }
                    }
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    complex_t delta_a_exp = !kIsVariableA ? cexp2f(delta_vals[i] * A_scaled) : delta_vals[i] * A_val;
                    weight_t B_delta_u_val = !kIsVariableB ? delta_vals[i] * float(u_vals[i]) : B_vals[i] * delta_vals[i] * float(u_vals[i]);
                    thread_data[i] = make_float4(delta_a_exp.real_, delta_a_exp.imag_, B_delta_u_val.real_, B_delta_u_val.imag_);
                    if (i == 0) {
                        smem_delta_a[threadIdx.x == 0 ? state_idx + (chunk % 2) * MAX_DSTATE : threadIdx.x + 2 * MAX_DSTATE] = delta_a_exp;
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    ddelta_vals[i] += ddelta_u * float(u_vals[i]) + (dx * conj(A_val) * a_conj).real_;
                    if constexpr (kIsVariableA) {
                        dA_vals[i] = delta_vals[i] * dx * a_conj;
                    } else {
                        dA_val += delta_vals[i] * dx * a_conj;
                    }
                    if constexpr (kIsVariableB) { dB_vals[i] = dx * delta_vals[i] * float(u_vals[i]); }
                ...
                // Block-exchange to make the atomicAdd's coalesced, otherwise they're much slower
                if constexpr (kIsVariableB || kIsVariableC || kIsVariableA) {
                    float dA_vals_f[kNItems * 2], dB_vals_f[kNItems * 2], dC_vals_f[kNItems * 2];
                    if constexpr (kIsVariableB) {
                        #pragma unroll
                        for (int i = 0; i < kNItems; ++i) {
                            dB_vals_f[i * 2] = dB_vals[i].real_;
                    ...
                    if constexpr (kIsVariableA) {
                        #pragma unroll
                        for (int i = 0; i < kNItems; ++i) {
                            dA_vals_f[i * 2] = dA_vals[i].real_;
                            dA_vals_f[i * 2 + 1] = dA_vals[i].imag_;
                        }
                        auto &smem_exchange_A = !kIsVariableC ? (!kIsVariableB ? smem_exchange : smem_exchange1) : (!kIsVariableB ? smem_exchange1 : smem_exchange2);
                        Ktraits::BlockExchangeT(smem_exchange_A).BlockedToStriped(dA_vals_f, dA_vals_f);
                    }
                    const int seqlen_remaining = (params.seqlen - chunk * kChunkSize) * 2 - threadIdx.x;
                    float *dA_cur = reinterpret_cast<float *>(dA) + state_idx * params.dA_dstate_stride + chunk * kChunkSize * 2 + threadIdx.x;
                    float *dB_cur = reinterpret_cast<float *>(dB) + state_idx * params.dB_dstate_stride + chunk * kChunkSize * 2 + threadIdx.x;
                    float *dC_cur = reinterpret_cast<float *>(dC) + state_idx * params.dC_dstate_stride + chunk * kChunkSize * 2 + threadIdx.x;
                    #pragma unroll
                    for (int i = 0; i < kNItems * 2; ++i) {
                        if (i * kNThreads < seqlen_remaining) {
                            if constexpr (kIsVariableA) { gpuAtomicAdd(dA_cur + i * kNThreads, dA_vals_f[i]); }
                            if constexpr (kIsVariableB) { gpuAtomicAdd(dB_cur + i * kNThreads, dB_vals_f[i]); }
                            if constexpr (kIsVariableC) { gpuAtomicAdd(dC_cur + i * kNThreads, dC_vals_f[i]); }
                        }
                    }
        ...
        Avar -= kChunkSize * (!kIsComplex ? 1 : 2);
        Bvar -= kChunkSize * (!kIsComplex ? 1 : 2);
        Cvar -= kChunkSize * (!kIsComplex ? 1 : 2);
    }
    if (params.dD_ptr != nullptr) {
...
template<typename Ktraits>
__global__ __launch_bounds__(Ktraits::kNThreads, Ktraits::kMinBlocks)
void selective_scan_decay_bwd_kernel(SSMDecayParamsBwd params) {
    constexpr bool kHasZ = Ktraits::kHasZ;
    constexpr int kNThreads = Ktraits::kNThreads;
    constexpr int kNItems = Ktraits::kNItems;
    using input_t = typename Ktraits::input_t;
    ...
    auto& smem_load_weight1 = *reinterpret_cast<typename Ktraits::BlockLoadWeightT::TempStorage*>(smem_ + sizeof(typename Ktraits::BlockLoadWeightT::TempStorage));
    auto& smem_load_weight2 = *reinterpret_cast<typename Ktraits::BlockLoadWeightT::TempStorage*>(smem_ + 2 * sizeof(typename Ktraits::BlockLoadWeightT::TempStorage));
    auto& smem_load_weight3 = *reinterpret_cast<typename Ktraits::BlockLoadWeightT::TempStorage*>(smem_ + 3 * sizeof(typename Ktraits::BlockLoadWeightT::TempStorage));
    auto& smem_store = reinterpret_cast<typename Ktraits::BlockStoreT::TempStorage&>(smem_);
    auto& smem_exchange = *reinterpret_cast<typename Ktraits::BlockExchangeT::TempStorage*>(smem_ + Ktraits::kSmemIOSize);
    auto& smem_exchange1 = *reinterpret_cast<typename Ktraits::BlockExchangeT::TempStorage*>(smem_ + Ktraits::kSmemIOSize + sizeof(typename Ktraits::BlockExchangeT::TempStorage));
    auto& smem_exchange2 = *reinterpret_cast<typename Ktraits::BlockExchangeT::TempStorage*>(smem_ + Ktraits::kSmemIOSize + 2 * (sizeof(typename Ktraits::BlockExchangeT::TempStorage)));
    auto& smem_exchange3 = *reinterpret_cast<typename Ktraits::BlockExchangeT::TempStorage*>(smem_ + Ktraits::kSmemIOSize + 3 * (sizeof(typename Ktraits::BlockExchangeT::TempStorage)));
    auto& smem_reduce = *reinterpret_cast<typename Ktraits::BlockReduceT::TempStorage*>(reinterpret_cast<char *>(&smem_exchange) + Ktraits::kSmemExchangeSize);
    auto& smem_reduce_float = *reinterpret_cast<typename Ktraits::BlockReduceFloatT::TempStorage*>(&smem_reduce);
    auto& smem_reduce_complex = *reinterpret_cast<typename Ktraits::BlockReduceComplexT::TempStorage*>(&smem_reduce);
    auto& smem_scan = *reinterpret_cast<typename Ktraits::BlockScanT::TempStorage*>(reinterpret_cast<char *>(&smem_reduce) + Ktraits::kSmemReduceSize);
    ...
    weight_t *C = reinterpret_cast<weight_t *>(params.C_ptr) + dim_id * params.C_d_stride;
    input_t *Cvar = reinterpret_cast<input_t *>(params.C_ptr) + batch_id * params.C_batch_stride + group_id * params.C_group_stride;
    weight_t *I = reinterpret_cast<weight_t *>(params.I_ptr) + dim_id * params.I_d_stride;
    input_t *Ivar = reinterpret_cast<input_t *>(params.I_ptr) + batch_id * params.I_batch_stride + group_id * params.I_group_stride;
    weight_t *dA = reinterpret_cast<weight_t *>(params.dA_ptr) + batch_id * params.dA_batch_stride + group_id * params.dA_group_stride;
    weight_t *dB = reinterpret_cast<weight_t *>(params.dB_ptr) + batch_id * params.dB_batch_stride + group_id * params.dB_group_stride;
    weight_t *dC = reinterpret_cast<weight_t *>(params.dC_ptr) + batch_id * params.dC_batch_stride + group_id * params.dC_group_stride;
    weight_t *dI = reinterpret_cast<weight_t *>(params.dI_ptr) + batch_id * params.dI_batch_stride + group_id * params.dI_group_stride;
    float *dD = params.dD_ptr == nullptr ? nullptr : reinterpret_cast<float *>(params.dD_ptr) + dim_id;
    float D_val = params.D_ptr == nullptr ? 0 : reinterpret_cast<float *>(params.D_ptr)[dim_id];
    float *ddelta_bias = params.ddelta_bias_ptr == nullptr ? nullptr : reinterpret_cast<float *>(params.ddelta_bias_ptr) + dim_id;
    float delta_bias = params.delta_bias_ptr == nullptr ? 0 : reinterpret_cast<float *>(params.delta_bias_ptr)[dim_id];
    ...
    delta += (params.n_chunks - 1) * kChunkSize;
    dout += (params.n_chunks - 1) * kChunkSize;
    Avar += (params.n_chunks - 1) * kChunkSize;
    Bvar += (params.n_chunks - 1) * kChunkSize;
    Cvar += (params.n_chunks - 1) * kChunkSize;
    Ivar += (params.n_chunks - 1) * kChunkSize;
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        input_t u_vals[kNItems];
        input_t delta_vals_load[kNItems];
        input_t dout_vals_load[kNItems];
        ...
        float dout_vals[kNItems], delta_vals[2][kNItems];
        #pragma unroll
        for (int i = 0; i < kNItems; ++i) {
            dout_vals[i] = float(dout_vals_load[i]);
            delta_vals[0][i] = float(delta_vals_load[i]) + delta_bias;
            delta_vals[1][i] = softplus(-delta_vals[0][i]);
            delta_vals[0][i] = softplus(delta_vals[0][i]);
        }
        ...
        float ddelta_vals[2][kNItems] = {0};
        __syncthreads();
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            weight_t A_scaled, I_scaled, A_val, B_val, C_val;
            weight_t A_vals[kNItems], B_vals[kNItems], I_vals[kNItems], C_vals[kNItems];
            load_weight<Ktraits, false>(Bvar + state_idx * params.B_dstate_stride, B_vals,
                smem_load_weight, (params.seqlen - chunk * kChunkSize));
            auto &smem_load_weight_C = smem_load_weight1;
            load_weight<Ktraits, false>(Cvar + state_idx * params.C_dstate_stride, C_vals,
                smem_load_weight_C, (params.seqlen - chunk * kChunkSize));
            auto &smem_load_weight_A =  smem_load_weight2;
            load_weight<Ktraits, false>(Avar + state_idx * params.A_dstate_stride, A_vals,
                smem_load_weight_A, (params.seqlen - chunk * kChunkSize));
            auto &smem_load_weight_I =  smem_load_weight3;
            load_weight<Ktraits, false>(Ivar + state_idx * params.I_dstate_stride, I_vals,
                smem_load_weight_I, (params.seqlen - chunk * kChunkSize));
            // const weight_t A_val = smem_a[state_idx];
            scan_t thread_data[kNItems], thread_reverse_data[kNItems];
            #pragma unroll
            for (int i = 0; i < kNItems; ++i) {
                constexpr float kLog2e = M_LOG2E;
                A_scaled = A_vals[i] * kLog2e;
                I_scaled = I_vals[i] * kLog2e;
                const float delta_a_exp =  exp2f(delta_vals[0][i] * A_scaled);
                thread_data[i] = make_float2(
                    delta_a_exp,
                    // delta_vals[i] * float(u_vals[i]) * B_vals[i]
                    exp2f(delta_vals[1][i] * I_scaled) * B_vals[i] * u_vals[i]
                );
                if (i == 0) {
                    smem_delta_a[threadIdx.x == 0 ? state_idx + (chunk % 2) * MAX_DSTATE : threadIdx.x + 2 * MAX_DSTATE] = delta_a_exp;
                } else {
            ...
            if (threadIdx.x == 0) { smem_running_postfix[state_idx] = postfix_op.running_prefix; }
            weight_t dA_val = 0, dBC_val = 0;
            weight_t dA_vals[kNItems], dB_vals[kNItems], dC_vals[kNItems], dI_vals[kNItems];
            #pragma unroll
            for (int i = 0; i < kNItems; ++i) {
                A_val = A_vals[i];
                constexpr float kLog2e = M_LOG2E;
                ...
                const float exp_neg_delta_i =  exp2f(delta_vals[1][i] * I_scaled);
                du_vals[i] += ddelta_u * exp_neg_delta_i; // delta_vals[i];
                const float a = thread_data[i].y -  exp_neg_delta_i * B_vals[i] * float(u_vals[i]);
                // a here is deltaA * x
                // dx * x * deltaA * A
                ddelta_vals[0][i] += A_val * dx * a;  
                // original: dx * B_vals[i] * float(u_vals[i])
                // new: dx * deltaB * B_vals[i] * float(u_vals[i]) * I_vals[i]
                const float ddelta_i = ddelta_u * float(u_vals[i]) * exp_neg_delta_i;
                ddelta_vals[1][i] += ddelta_i * I_vals[i];  
                dI_vals[i] = ddelta_i * delta_vals[1][i];
                dA_vals[i] = delta_vals[0][i] * dx * a;
                dB_vals[i] = dx * float(u_vals[i]) * exp_neg_delta_i; // * delta_vals[1][i];
            ...
            auto &smem_exchange_A =  smem_exchange2;
            Ktraits::BlockExchangeT(smem_exchange_A).BlockedToStriped(dA_vals, dA_vals);

            auto &smem_exchange_I =  smem_exchange3;
            Ktraits::BlockExchangeT(smem_exchange_I).BlockedToStriped(dI_vals, dI_vals);

            const int seqlen_remaining = params.seqlen - chunk * kChunkSize - threadIdx.x;
            weight_t *dA_cur = dA + state_idx * params.dA_dstate_stride + chunk * kChunkSize + threadIdx.x;
            weight_t *dI_cur = dI + state_idx * params.dI_dstate_stride + chunk * kChunkSize + threadIdx.x;
            weight_t *dB_cur = dB + state_idx * params.dB_dstate_stride + chunk * kChunkSize + threadIdx.x;
            weight_t *dC_cur = dC + state_idx * params.dC_dstate_stride + chunk * kChunkSize + threadIdx.x;
            #pragma unroll
            for (int i = 0; i < kNItems; ++i) {
                if (i * kNThreads < seqlen_remaining) {
                    { gpuAtomicAdd(dA_cur + i * kNThreads, dA_vals[i]); }
                    { gpuAtomicAdd(dI_cur + i * kNThreads, dI_vals[i]); }
        ...
        for (int i = 0; i < kNItems; ++i) {
            float delta_val = float(delta_vals_load[i]) + delta_bias;
            ddelta_vals[0][i] = dsoftplus(delta_val, ddelta_vals[0][i]);
            ddelta_vals[0][i] -= dsoftplus(-delta_val, ddelta_vals[1][i]);
            // ddelta_vals[1][i] = -dsoftplus(-delta_val, ddelta_vals[1][i]);
        }
        for (int i = 0; i < kNItems; ++i) { ddelta_bias_val += ddelta_vals[0][i]; }
        ...
        store_output<Ktraits>(du, du_vals, smem_store, params.seqlen - chunk * kChunkSize);
        __syncthreads();
        store_output<Ktraits>(ddelta, ddelta_vals[0], smem_store, params.seqlen - chunk * kChunkSize);
...
template<int kNThreads, int kNItems, typename input_t, typename weight_t>
void selective_scan_new_bwd_launch(SSMNewParamsBwd &params, cudaStream_t stream) {
    BOOL_SWITCH(params.seqlen % (kNThreads * kNItems) == 0, kIsEvenLen, [&] {
        BOOL_SWITCH(params.is_variable_A, kIsVariableA, [&] {
            BOOL_SWITCH(params.is_variable_B, kIsVariableB, [&] {
                BOOL_SWITCH(params.is_variable_C, kIsVariableC, [&] {
                    BOOL_SWITCH(params.delta_softplus, kDeltaSoftplus, [&] {
                        BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
                            using Ktraits = Selective_Scan_new_bwd_kernel_traits<kNThreads, kNItems, kIsEvenLen,kIsVariableA, kIsVariableB, kIsVariableC, kDeltaSoftplus, kHasZ, input_t, weight_t>;
                            // using Ktraits = Selective_Scan_bwd_kernel_traits<kNThreads, kNItems, true, kIsVariableB, kIsVariableC, kDeltaSoftplus, kHasZ, input_t, weight_t>;
                            // TODO: check this
                            constexpr int kSmemSize = Ktraits::kSmemSize + MAX_DSTATE * sizeof(typename Ktraits::scan_t) + (kNThreads + 4 * MAX_DSTATE) * sizeof(typename Ktraits::weight_t);
                            // printf("smem_size = %d\n", kSmemSize);
                            dim3 grid(params.batch, params.dim);
                            auto kernel = &selective_scan_new_bwd_kernel<Ktraits>;
                            if (kSmemSize >= 48 * 1024) {
                                C10_CUDA_CHECK(cudaFuncSetAttribute(
                                    kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, kSmemSize));
                            }
...
template<int kNThreads, int kNItems, typename input_t, typename weight_t>
void selective_scan_decay_bwd_launch(SSMDecayParamsBwd &params, cudaStream_t stream) {
    BOOL_SWITCH(params.seqlen % (kNThreads * kNItems) == 0, kIsEvenLen, [&] {
        BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
            using Ktraits = Selective_Scan_decay_bwd_kernel_traits<kNThreads, kNItems, kIsEvenLen, kHasZ, input_t, weight_t>;
            // using Ktraits = Selective_Scan_bwd_kernel_traits<kNThreads, kNItems, true, kIsVariableB, kIsVariableC, kDeltaSoftplus, kHasZ, input_t, weight_t>;
            // TODO: check this
            constexpr int kSmemSize = Ktraits::kSmemSize + MAX_DSTATE * sizeof(typename Ktraits::scan_t) + (kNThreads + 4 * MAX_DSTATE) * sizeof(typename Ktraits::weight_t);
            // printf("smem_size = %d\n", kSmemSize);
            dim3 grid(params.batch, params.dim);
            auto kernel = &selective_scan_decay_bwd_kernel<Ktraits>;
            if (kSmemSize >= 48 * 1024) {
                C10_CUDA_CHECK(cudaFuncSetAttribute(
                    kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, kSmemSize));
            }
...
template<typename input_t, typename weight_t>
void selective_scan_new_bwd_cuda(SSMNewParamsBwd &params, cudaStream_t stream) {
    if (params.seqlen <= 128) {
        selective_scan_new_bwd_launch<32, 4, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 256) {
        selective_scan_new_bwd_launch<32, 8, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 512) {
        selective_scan_new_bwd_launch<32, 16, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 1024) {
        selective_scan_new_bwd_launch<64, 16, input_t, weight_t>(params, stream);
    } else {
        selective_scan_new_bwd_launch<128, 16, input_t, weight_t>(params, stream);
    }
}
template<typename input_t, typename weight_t>
void selective_scan_decay_bwd_cuda(SSMDecayParamsBwd &params, cudaStream_t stream) {
    if (params.seqlen <= 128) {
        selective_scan_decay_bwd_launch<32, 4, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 256) {
        selective_scan_decay_bwd_launch<32, 8, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 512) {
        selective_scan_decay_bwd_launch<32, 16, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 1024) {
        selective_scan_decay_bwd_launch<64, 16, input_t, weight_t>(params, stream);
    } else {
        selective_scan_decay_bwd_launch<128, 16, input_t, weight_t>(params, stream);
    }
}